...
export async function up({ db, payload, req }: MigrateArgs): Promise<void> {
  ...
  const srcCheck = await db.execute(sql`SELECT COUNT(*)::int AS cnt FROM "collections" WHERE "name" IS NOT NULL AND "name" != '';`)
  const srcCount = srcCheck?.rows?.[0]?.cnt ?? 0
  if (count < 1 && srcCount > 0) throw new Error(`[UP] Backfill inserted 0 rows — ${srcCount} source row(s) had non-empty name. Possible data loss.`);
